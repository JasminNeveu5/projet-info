...
def tightestrace():
    ...
    merged_result = pd.merge(results.query("positionOrder == 1"), results.query("positionOrder == 2"), on="raceId")[["raceId", "milliseconds_x", "milliseconds_y"]]
    merged_result = merged_result[~merged_result.isin(['\\N']).any(axis=1)]
    merged_result["time_diff"] = merged_result["milliseconds_y"].astype(int) - merged_result["milliseconds_x"].astype(int)
    merged_result_races = pd.merge(merged_result, races, on ="raceId")[["name","year","date","time_diff"]]
    res = merged_result_races.loc[merged_result_races["time_diff"].idxmin()]
    return Race(name = res["name"], year = int(res["year"]),date = res["date"],time_diff =res["time_diff"])
