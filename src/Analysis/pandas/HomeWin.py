...
def home_win(nationalite):
    ...
    foreign_races_moyenne = foreign_races.groupby("driverId").agg(moyenne_etrangere = ("positionOrder", "mean"))

    compare = pd.merge(local_races_moyenne,position_moyenne_pilote, on="driverId").merge(foreign_races_moyenne, on="driverId", how="left")
    compare = compare[["driverId","forename","surname","moyenne_overall","moyenne_etrangere","moyenne_locale","nationality"]].query(f"nationality == '{nationalite}'")
